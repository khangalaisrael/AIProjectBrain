...
import { Bot, FileCode2, Loader2, MessageSquare, Send, User, X } from "lucide-react";
...
export function ChatDock() {
  ...
  // Default the selector to the first ready repository.
  useEffect(() => {
      ...
      <AnimatePresence>
        {open && (
          <motion.aside
            ...
            className="border-border bg-background fixed inset-y-0 right-0 z-50 flex w-full max-w-md flex-col border-l shadow-xl"
          >
            {/* Header */}
...
function MessageBubble({ message }: { message: Message }) {
  ...
  return (
    <div className={cn("flex gap-2", isUser && "flex-row-reverse")}>
      ...
      <div className={cn("min-w-0 space-y-2", isUser ? "text-right" : "text-left")}>
        <div
          className={cn(
